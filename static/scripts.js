...
/* ========================= */
/* GALERÍA DINÁMICA (CON RENOVACIÓN ALEATORIA) */
/* ========================= */
function initGallery(total = 30) {
    gallery.innerHTML = "";
    // Generamos un número aleatorio para que las fotos cambien en cada ciclo
    const seedOffset = Math.floor(Math.random() * 1000);
    
    for (let i = 0; i < total; i++) {
        const div = document.createElement("div");
        div.className = "slide";
        div.style.backgroundImage = `url(https://picsum.photos/1920/1080?nature&sig=${seedOffset + i})`;
...
async function cargarSesiones() {
    try {
        ...
        console.log("Sesiones cargadas con éxito");
    } catch (e) { 
        console.error("Error al cargar el JSON. Revisa el nombre del archivo."); 
    }
}
...
async function mostrarBloque() {
    ...
    if (sesiones.length === 0) return; // Esperar a que carguen los datos
    ...
    nextBtn.style.display = "inline-block"; // Siempre visible para el ciclo infinito
    ...
    } catch (e) { console.log("Error en flujo."); }
}

/* ========================= */
/* INICIO AUTOMÁTICO DE DATOS */
/* ========================= */
cargarSesiones(); // Cargar datos apenas abre la web
...
nextBtn.onclick = () => {
    ...
    if (currentBloque < bloquesActuales.length - 1) {
        ...
        // CICLO INFINITO: Reiniciar todo y cambiar fotos
        currentSesion = 0;
        currentBloque = 0;
        initGallery(30); 
    }

...
function hablar(texto, soft=false) {
    return new Promise(resolve => {
        ...
        const limpio = texto.replace(/<[^>]*>/g, ""); // Eliminar HTML
...
async function respirar(texto, duracion) {
    ...
    if(t.includes("inhala")) inicio = 1, fin = 1.6;
    if(t.includes("retiene")) inicio = 1.6, fin = 1.6;
    if(t.includes("exhala")) inicio = 1.6, fin = 0.8;
    if(t.includes("respira")) respirarAutomatico();
...
function decision(bloque) {
    return new Promise(resolve => {
        limpiarBloque();
        breathText.innerText = bloque.pregunta;

        bloque.opciones.forEach((opt, i) => {
            const btn = document.createElement("button");
            btn.innerText = opt;
            btn.style.cssText = "display:block;margin:10px auto;padding:10px;border-radius:8px;border:1px solid #00d2ff;background:transparent;color:white;cursor:pointer;";
            btn.onclick = async () => {
                if (i === bloque.correcta) {
                    breathText.innerText = "✅ Correcto\n" + bloque.explicacion;
                    await hablar("Correcto. " + bloque.explicacion);
                    puntos += bloque.recompensa || 0;
                    resolve();
                } else {
                    breathText.innerText = "❌ Incorrecto\n" + bloque.explicacion;
...
async function mostrarBloque(bloque) {
    ...
    switch(bloque.tipo) {
        ...
        case "tvid_ejercicio_largo":
            for(const txt of bloque.textos) await escribirTextoYHablar(txt, bloque.color);
            break;
        case "cierre":
            await escribirTextoYHablar(bloque.texto, bloque.color);
